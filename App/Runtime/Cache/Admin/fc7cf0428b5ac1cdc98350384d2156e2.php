...
<!--        <div  data-options="region:'south',split:true" style="height:50px;">
    ...
    <div class="easyui-panel" title="添加管理员"  data-options="fit:true,border:false">
        <div style="padding:10px 60px 20px 60px">
            <form id="ff" method="post" class="admin-form" action="/ajax/submit/">
                ...
                <div class="input-group">
                    <label>角色:</label>
                    <select name="gid">
                        <?php if(is_array($fname)): $i = 0; $__LIST__ = $fname;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($v["id"]); ?>" <?php if($v["id"] == $Rs['gid']): ?>selected ="selected"<?php endif; ?>><?php echo ($v["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                </div>
                <div class="input-group">
                    <label>登入名:</label>
                    <input class="easyui-validatebox mid-text" value="<?php echo ($Rs['account']); ?>" type="text" name="account" ></input>
                </div>
                <div class="input-group">
                    <label>昵称:</label>
                    <input name="nickname" id="nickname" type="text" size="20" value="<?php echo ($Rs['nickname']); ?>" >
                </div>
                <div class="hasEditor hasPic" style="margin-top:20px;">
                    <label >头像：</label>
                    <div id="tdimgcover" style="display: inline-block">
                        <div class="showImg" >
                            <div id="disimgcover" style='width:33px;height:33px;' class="showImgC">
                                <?php echo ($Rs['favatar']); ?>
                            </div>
                            <div id="btnimgcover" style="margin-top:10px;">
                                <button onclick="$('#tag-window').window('open');
                                            $('#tag-window').window('refresh', '/admin/avatar.html');" type="button" class="btn btn-primary">设置头像</button>
                                <div id="tag-window" class="easyui-window" title="设置头像" style="width:700px;height:500px"
                ...
                <div class="input-group">
                    <label>重复密码:</label>
                    <input name="repassword"  id="repassword" type="password" size="20"  >
                </div>
                <div class="input-group">
                    <label>是否启用:</label>
                    <?php echo ($fisopen); ?>
