...
/**
 * Encoding     :   UTF-8
 * Created on   :   2014-4-6 21：41 by 曹轩铭 , 280046197@qq.com
 * For          :   时尚专栏
 */
class HealthcareAction extends PublicAction {

    function index() {
        $typename = "养生"; //完全可以修改成传参加载不同的CSS
        $VideoMob = D("Video");
        ...
        $Focus = D('Focus');
        $focusCat = $FocusCat->where("name = '$typename'")->find();
    ...
    function HealthCarelist() {
        ...
        switch ($M) {
            case 'News':
                $this->display("HealthCare/News");
                break;
            case 'Picture':
                $this->display("HealthCare/Picture");
                break;
            case 'Video':
                $this->display("HealthCare/Video");
                break;
            case 'Newscat':
                $this->display("HealthCare/Newscat");
                break;
            case 'VideoCat':
                $this->display("HealthCare/Videocat");
                break;
        }
    }
