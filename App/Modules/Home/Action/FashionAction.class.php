...
/**
 * Encoding     :   UTF-8
 * Created on   :   2014-4-7 19：20 by 曹轩铭 , 280046197@qq.com
 * For          :   时尚专栏
 */
class FashionAction extends PublicAction {

    function index() {
        $typename = "时尚"; //完全可以修改成传参加载不同的CSS
        $VideoMob = D("Video");
    ...
    function Fashionlist() {
        ...
        switch ($M) {
            case 'News':
                $this->display("Fashion/News");
                break;
            case 'Picture':
                $this->display("Fashion/Picture");
                break;
            case 'Video':
                $this->display("Fashion/Video");
                break;
            case 'Newscat':
                $this->display("Fashion/Newscat");
                break;
            case 'VideoCat':
                $this->display("Fashion/Videocat");
                break;
        }
    }
