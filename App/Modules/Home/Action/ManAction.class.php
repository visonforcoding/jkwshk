...
/**
 * Encoding     :   UTF-8
 * Created on   :   2014-3-31 15:09:20 by 曹轩铭 , 280046197@qq.com
 * For          :   男性专栏
 */
class ManAction extends Action {

    function index() {
        $typename = "男性"; //完全可以修改成传参加载不同的CSS
        $VideoMob = D("Video");
        ...
        $Focus = D('Focus');
        $focusCat = $FocusCat->where("name = '男性'")->find();
        ...
        $videoCatList = $VideoCatMob->where("type like '%$typeid%' and zhuanjino = '1'")
                ->limit(8)
                ->select();
        /* 热门数据 */
    ...
    function manlist() {
       ...
       $show = $Page->pageShow();
        ...
        switch ($M) {
            case 'News':
                $this->display("Man/News");
                break;
            case 'Picture':
                $this->display("Man/Picture");
                break;
            case 'Video':
                $this->display("Man/Video");
                break;
            case 'Newscat':
                $this->display("Man/Newscat");
                break;
            case 'VideoCat':
                $this->display("Man/Videocat");
                break;
        }
    }
