...
/**
 * Encoding     :   UTF-8
 * Created on   :   2014-4-6 16:11 by 曹轩铭 , 280046197@qq.com
 * For          :   慢病专栏
 */
class ChronicdisAction extends Action {

    function index() {
        $typename = "慢病"; //完全可以修改成传参加载不同的CSS
        $VideoMob = D("Video");
        ...
        $Focus = D('focus');
        $focusCat = $FocusCat->where("name = '慢病'")->find();
    ...
    function Chronicdislist() {
        ...
        switch ($M) {
            case 'News':
                $this->display("Chronicdis/News");
                break;
            case 'Picture':
                $this->display("Chronicdis/Picture");
                break;
            case 'Video':
                $this->display("Chronicdis/Video");
                break;
            case 'Newscat':
                $this->display("Chronicdis/Newscat");
                break;
            case 'VideoCat':
                $this->display("Chronicdis/Videocat");
                break;
        }
    }
