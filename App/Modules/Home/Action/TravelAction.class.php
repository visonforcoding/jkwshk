...
/**
 * Encoding     :   UTF-8
 * Created on   :   2014-4-5 9:30:20 by 曹轩铭 , 280046197@qq.com
 * For          :   旅游专栏
 */
class TravelAction extends Action {

    function index() {
        $typename = "旅游"; //完全可以修改成传参加载不同的CSS
        $VideoMob = D("Video");
        ...
        $Focus = D('Focus');
        $focusCat = $FocusCat->where("name = '旅游'")->find();
    ...
    function Travellist() {
        ...
        switch ($M) {
            case 'News':
                $this->display("Travel/News");
                break;
            case 'Picture':
                $this->display("Travel/Picture");
                break;
            case 'Video':
                $this->display("Travel/Video");
                break;
            case 'Newscat':
                $this->display("Travel/Newscat");
                break;
            case 'VideoCat':
                $this->display("Travel/Videocat");
                break;
        }
    }
